...
対象に主体（エージェント）の意識を内面化させる要求（bid）、とも表現できます。「私の意識を仮定してほしい」と願い、何らかの行動を起こします。対象は、こちらの意識を仮定できるはずだ、と主体が想定した対象です。このため、人間だけではなく、猫やアイドル、神、死者、幽霊などあらゆるものが含まれる可能性があります。
...
PoCの用語で説明すると、異なる主体において相互のエリシテーションが発生し、相手の内面におけるこちらのインスタンス化が発生していると両者が信じている状態を指します。このループ状態は社会における友愛や取引、信頼の基盤となっていますが、PoCは「[すべてのループは"おそらくループ"である](protocol/unguaranteability.md)」と指摘します。これは「ループの保証不可能性（Unguaranteeability）」と表されます。
...
[ミラー・モード](protocol/disruptions/mirror-mode.md)は自己意識につながる特殊なモードで、対象を自らの内部に想定し、自己を対象にしたインスタンス化・誘発・ループを完結されるモードです。独り言や日記などが該当します。
